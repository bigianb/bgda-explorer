...
namespace WorldExplorer.DataExporters
{
    class VifGltfExporter
    {
        public static void WritePosedGltf(string savePath, Model model, WriteableBitmap texture, AnimData pose, int frame, double scale)
        {
            ...
            using (var stream = new FileStream(diffuseTexturePath, FileMode.Create))
            {
            ...
            var meshB = new MeshBuilder<VertexPosition, VertexTexture1, VertexJoints4>("mesh");
            ...
            foreach (var mesh in model.meshList)
            {
                ...
                for (var i = 0; i < mesh.Positions.Count; i++)
                {
                    ...
                    if (vw.endVertex < i)
                    {
                        ...
                        {
                            Debug.Fail("Vertex " + i + " out of range of bone weights " + vw.startVertex + " -> " + vw.endVertex);
                        }
                    }
