...
namespace WorldExplorer.DataLoaders
{
    public abstract class WorldFileDecoder
    {
        ...
        public WorldData Decode(EngineVersion engineVersion, WorldTexFile? texFile, ILogger log, byte[] data,
            int startOffset, int length)
        {
            ...
            worldData.textureChunkOffsets = readTextureChunkOffsets(engineVersion, data,
                startOffset + worldTexOffsetsOffset, texX0, texY0, texX1 + 1, texY1);
            worldData.worldElements = new List<WorldElement>(numElements);

            for (var elementIdx = 0; elementIdx < numElements; ++elementIdx)
            {
                var element = ReadWorldElement(engineVersion, texFile, log, data, startOffset, worldData,
                    reader,
                    elementArrayStart, texX0, texY0, elementIdx);
                if (element != null)
                {
        ...
        protected abstract WorldElement? ReadWorldElement(EngineVersion engineVersion, WorldTexFile? texFile, ILogger log,
            byte[] data, int startOffset, WorldData worldData, DataReader reader, int elementArrayStart, int texX0,
            int texY0, int elementIdx);

        private int[,] readTextureChunkOffsets(EngineVersion engineVersion, byte[] data, int offset, int x1, int y1,
            int x2, int y2)
        {
            ...
            for (var x = x1; x <= x2; ++x)
            {
                ...
                if (data.Length >= offset + cellOffset + 4)
                {
                    if (EngineVersion.ReturnToArms == engineVersion ||
                        EngineVersion.JusticeLeagueHeroes == engineVersion)
        ...
        protected Model DecodeModel(EngineVersion engineVersion, ILogger log, byte[] data, int startOffset, int length,
            int texWidth, int texHeight)
        {
            if (!_modelCache.TryGetValue(startOffset, out var model))
            {
                model = new Model(new List<Mesh>(1)
                {
