...
public class RtaAnmDecoder : AnmDecoder
{
    ...
    public override AnimData Decode(ReadOnlySpan<byte> data)
    {
        ...
        for (var boneNum = 0; boneNum < numBones; ++boneNum)
        {
            ...
            var a = bitReader.ReadSigned(rotLen) / 4096.0f;
            var b = bitReader.ReadSigned(rotLen) / 4096.0f;
            var c = bitReader.ReadSigned(rotLen) / 4096.0f;
            var d = bitReader.ReadSigned(rotLen) / 4096.0f;
        ...
        while (bitReader.HasData(22) && frameNumber < maxFrames)
        {
            ...
            if (flag == 1)
            {
                ...
                Vector3D posDelta = new(prevVel.X * coEff, prevVel.Y * coEff, prevVel.Z * coEff);
            ...
            else
            {
                ...
                var coEff = (frameNumber - curAngVelFrame[boneNum]) / 131072.0f;
